 ...
 * 种子提取机配方：作物 → 种子。
 *
 * 配方配置由 Rebar 从插件资源的 `recipes/endfield-industry/seed_extractor.yml` 自动加载
 * （数据目录同名文件可覆盖，便于服主调整）。
 */
class SeedExtractorRecipe(
        ...
        return Gui.builder()
            ...
            .addIngredient('p', ItemButton.of(
                ItemStackBuilder.of(Material.FURNACE)
                    .name(Component.translatable("endfield-industry.gui.seed_extractor.title"))
                    .build()
            ))
    ...
    companion object {
        @JvmField
        val RECIPE_TYPE: ConfigurableRecipeType<SeedExtractorRecipe> =
            object : ConfigurableRecipeType<SeedExtractorRecipe>(EndfieldIndustryKeys.SEED_EXTRACTOR_RECIPE) {
                override fun loadRecipe(key: NamespacedKey, section: ConfigSection): SeedExtractorRecipe {
                    ...
                    return SeedExtractorRecipe(key, input, output)
                }
            }.apply { register() }
    }
}
